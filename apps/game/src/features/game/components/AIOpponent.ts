...
class PongAI {
  ...
  private predictBallTrajectory(
    ...
    while (simX < paddleX && bounceCount < 3) {
      const timeToReachPaddle = (paddleX - simX) / simVx; // 計算到達 paddle 需要的時間
      ...
      if (potentialY < 0 || potentialY > 1) {
        ...
        const timeToWall = potentialY < 0 ? -simY / simVy : (1 - simY) / simVy;
  ...
  */
  private calculateMove(
    ...
    else if (diff < -deadzone) direction = -1; // Move up
  ...
  private evaluatePowerUp(gameState: State, prediction: any): boolean {
    ...
    let useProbability = 0.05; // Base 5%

    if (scoreDiff < 0) useProbability += 0.1; // Losing
    if (prediction.confidence < 0.3) useProbability += 0.15; // Uncertain
    if (ballSpeed > 0.6) useProbability += 0.1; // Fast ball

    return Math.random() < useProbability;
  }

  setDifficulty(difficulty: number): void {
    // how often AI checks game status
  ...
  forceStrategy(
    strategy: "aggressive" | "defensive" | "adaptive" | "auto"
  ): void {
    if (strategy === "auto") {
